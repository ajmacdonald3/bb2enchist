...
#'
#' @param resight_data A dataframe with columns for ResightDate and FlagID (usually generated by \code{format_bandedbirds()}).
...
#filter out any rows where both cells are NA, keep any that have only one NA
data <- resight_data %>%
  filter_at(.vars = vars(FlagID, ResightPeriod),
